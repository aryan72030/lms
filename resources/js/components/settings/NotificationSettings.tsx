...
export default function NotificationSettings({ settings }: Props) {
    ...
    const [formData, setFormData] = useState({
        ...
        email_instructor_notifications: settings.email_instructor_notifications ?? true,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.post('/admin/settings/notification', formData, {
            preserveScroll: true,
            onSuccess: () => {
                notificationMessages.success('save');
            },
            onError: () => {
    ...
    const handleTestEmail = async () => {
        ...
        if (!formData.smtp_host || !formData.smtp_port || !formData.smtp_username || !formData.smtp_password || !formData.mail_from_address) {
            notificationMessages.error('test', 'Email test');
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Card>
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-2 items-end">
                            ...
                            <Button
                                ...
                                disabled={isTestingEmail || !formData.email_notifications_enabled}
                            >
                                {isTestingEmail ? (
                    ...
                    {/* Email Notification Types */}
                    <div className="space-y-4">
                        <h4 className="text-lg font-medium">Email Notification Types</h4>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {[
                                { key: 'email_user_registration', label: 'User Registration', desc: 'Send welcome emails to new users' },
                                { key: 'email_course_enrollment', label: 'Course Enrollment', desc: 'Notify students when they enroll in courses' },
                                { key: 'email_payment_confirmation', label: 'Payment Confirmation', desc: 'Send payment receipts and confirmations' },
                                { key: 'email_course_completion', label: 'Course Completion', desc: 'Send certificates and completion notifications' },
                                { key: 'email_password_reset', label: 'Password Reset', desc: 'Send password reset links' },
                                { key: 'email_instructor_notifications', label: 'Instructor Notifications', desc: 'Notify instructors about course activities' },
                            ].map((item) => (
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <Switch
                            ...
                            onCheckedChange={(checked) => handleInputChange('slack_notifications_enabled', checked)}
                        />
                    </div>
                    ...
                    {/* Slack Configuration */}
                    <div className="space-y-4">
                        <h4 className="text-lg font-medium">Slack Configuration</h4>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-start">
                                <Button
                                    ...
                                    disabled={isTestingSlack || !formData.slack_notifications_enabled}
                                >
                                    {isTestingSlack ? (
                                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-gray-900 mr-2"></div>
                                    ) : (
                    ...
                    {/* Slack Notification Types */}
                    <div className="space-y-4">
                        <h4 className="text-lg font-medium">Slack Notification Types</h4>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {[
                                { key: 'slack_user_registration', label: 'User Registration', desc: 'Notify when new users register' },
                                { key: 'slack_course_submission', label: 'Course Submission', desc: 'Notify when courses are submitted for review' },
                                { key: 'slack_payment_transactions', label: 'Payment Transactions', desc: 'Notify about payment activities' },
                            ].map((item) => (
                                <div key={item.key} className="flex items-center justify-between p-3 border rounded-lg">
                                    <div className="space-y-0.5">
                                        <Label className="text-sm font-medium">{item.label}</Label>
